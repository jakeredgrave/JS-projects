...
function addExpense(event)  {
    ...
    totalExpensesValue.innerHTML = "Total Price of Expenses: " + totalValue;
...
function removeExpense(event) {
    if (event.target.classList.contains("delete_button")) { //makes removeExpense() activate only when clicking a delete button, not whole <td> element
             ...
             for(let i = 0; i < cells.length; i++) {
                if(cells[i].classList.contains("price")) {
                   ...
                   // GOES THROUGH valuesToSum array IN ORDER TO FIND A VALUE EQUAL TO THE "PRICE" value IN "cells[i]"; then 
                    ...
                    totalExpensesValue.innerHTML =  "Total Price of Expenses: " + valueAfterSubtraction;
